# Roadmap — eb. Platform
**Derniere mise a jour :** 2026-04-27 — **Collection v5 : +3 fiches villa (Esmeralda, Tourlos Breeze, Santorini Estate)** sur `/collection/[slug]`, **Contact V4 (preselect villa + preselect yacht)**, **Yacht banner refonte** (image + position dans la page), **Hero overlays alleges sitewide** (bg-black/25), **IntentCards price ranges** sur les 3 portes B2C, **CSS reveal fallback** (animation auto a 2s si JS rate). Total : 5 fiches villa publiques (You & Me + Celestia + Esmeralda + Tourlos Breeze + Santorini Estate). Adjacent villa ne s'affiche QUE pour You & Me ↔ Celestia (relation physique reelle). Memoire saved : island name sells / villa floor €1,000 / mobile-first check systematique / must show signature features (gym pour footballers, kitchen, hot tub, hammam, chapel, cinema).
