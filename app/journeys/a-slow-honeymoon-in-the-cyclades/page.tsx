...
export default function SlowHoneymoonPage() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => { if (entry.isIntersecting) entry.target.classList.add('eb-visible'); });
      },
  ...
  const days = [
    { day: '01', title: 'Arrival. The first night on Milos.', body: 'Private transfer from your Athens hotel to the airport. Flight to Milos, then a private drive to a villa above the sea. Welcome dinner on the terrace.' },
    ...
    <main className="flex flex-col min-h-screen bg-white">
      ...
      <div className="absolute top-[88px] md:top-[96px] left-6 md:left-12 z-40">
        <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-[10px] md:text-[11px] tracking-[0.25em] uppercase text-white/75 font-light">
          ...
          <span className="opacity-70">A Slow Honeymoon</span>
        </nav>
      </div>

      <section data-nav-dark className="relative w-full h-screen min-h-[600px] overflow-hidden eb-image-vignette">
        <Image src="/MILOS_01.jpg" alt="Milos coast at golden hour, Cyclades" fill priority className="object-cover" />
        ...
        <div className="absolute bottom-14 md:bottom-20 left-6 md:left-12 right-6 md:right-12 text-white">
          <div className="bg-white/15 backdrop-blur-sm border border-white/30 rounded-full px-3 py-1 text-[10px] md:text-[11px] tracking-[0.2em] uppercase font-light inline-block mb-6 eb-fade-up">Private Journey &middot; 05</div>
      ...
      <section className="bg-white py-14 md:py-20">
        <div className="max-w-[1400px] mx-auto px-6 md:px-10">
          ...
          <div className="relative aspect-[16/9] md:aspect-[21/9] overflow-hidden group eb-fade-up eb-image-vignette max-w-[1200px] mx-auto">
            <Image src="/MILOS_02.jpg" alt="Milos turquoise cove and white rocks" fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
            <div className="absolute top-5 left-5 md:top-8 md:left-8 z-[3]"><span className="bg-white/15 backdrop-blur-sm border border-white/30 rounded-full px-3 py-1 text-[10px] tracking-[0.2em] uppercase text-white font-light">Two Islands</span></div>
            <div className="absolute bottom-6 left-6 right-6 md:bottom-10 md:left-10 md:right-10 text-white z-[3] max-w-[600px]">
              <h3 className="font-anton font-normal uppercase text-[26px] md:text-[40px] leading-[1] mb-3">Ten days, two homes.</h3>
              <p className="text-[13px] md:text-[15px] opacity-90 leading-[1.5] font-light">Milos for the coves. Folegandros for the silence.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section data-nav-dark className="relative w-full h-[80vh] min-h-[500px] overflow-hidden eb-image-vignette">
        <Image src="/MILOS_03.jpg" alt="A Cycladic villa above the sea" fill className="object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
      ...
      <section className="bg-white py-10 md:py-14 eb-fade-up">
        <div className="max-w-[1280px] mx-auto px-6 md:px-10">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-3">
            <div className="relative aspect-[4/3] sm:aspect-[3/4] overflow-hidden eb-image-vignette"><Image src="/MILOS_01.jpg" alt="Milos coast, Cyclades" fill className="object-cover" /></div>
            <div className="relative aspect-[4/3] sm:aspect-[3/4] overflow-hidden eb-image-vignette"><Image src="/MILOS_02.jpg" alt="Milos turquoise cove" fill className="object-cover" /></div>
            <div className="relative aspect-[4/3] sm:aspect-[3/4] overflow-hidden eb-image-vignette"><Image src="/MILOS_03.jpg" alt="Milos villa detail" fill className="object-cover" /></div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr] gap-10 md:gap-20">
              <div>
                ...
                <ul className="space-y-4">
                  {includedItems.map((item, i) => (<li key={i} className="flex items-start gap-4 text-[14px] md:text-[15px] text-[#1a1a1a]/75 font-light leading-[1.65]"><span className="inline-block w-[6px] h-[6px] bg-[#2e5a88] mt-[9px] shrink-0" /><span>{item}</span></li>))}
                </ul>
              </div>
              <div>
                <div className="text-[11px] tracking-[0.3em] uppercase text-[#1a1a1a]/50 font-light mb-6">On Request</div>
                <ul className="space-y-4">
                  {onRequestItems.map((item, i) => (<li key={i} className="flex items-start gap-4 text-[13px] md:text-[14px] text-[#1a1a1a]/60 font-light leading-[1.6]"><span className="inline-block w-[5px] h-[5px] border border-[#1a1a1a]/25 mt-[8px] shrink-0" /><span>{item}</span></li>))}
                </ul>
            ...
            {[
              ...
              <Link key={i} href={card.href} className="relative aspect-[3/4] overflow-hidden cursor-pointer group block eb-image-vignette">
                ...
                <div className="absolute bottom-6 left-6 right-6 text-white transition-transform duration-500 group-hover:-translate-y-2 z-[3]">
                  <h3 className="font-anton font-normal uppercase text-[28px] md:text-[32px] leading-[0.95] mb-2">{card.title}</h3>
      ...
      <section data-nav-dark className="relative w-full h-[70vh] min-h-[480px] overflow-hidden eb-image-vignette">
        ...
        <div className="relative z-[3] h-full flex items-center justify-center px-8 md:px-10">
          <div className="text-center max-w-[640px] eb-fade-in">
            ...
            <div className="flex justify-center"><Link href="/contact?type=journey&journey=a-slow-honeymoon-in-the-cyclades" className="bg-white text-[#2e5a88] px-8 py-3.5 rounded-full text-[10px] font-medium tracking-[0.25em] uppercase transition-all hover:scale-[1.03] hover:shadow-xl">Start the conversation &rarr;</Link></div>
          </div>
        </div>
      </section>
