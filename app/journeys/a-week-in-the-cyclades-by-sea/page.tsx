...
export default function WeekBySeaPage() {
  const [openDay, setOpenDay] = useState<number | null>(null);


  const days = [
    { day: '01', title: 'Boarding. Athens to the Aegean.', body: 'Private transfer from your Athens hotel to the marina. Welcome on board the yacht, settling into your cabin, early dinner on deck as the lights of Athens fade behind.' },
    { day: '02', title: 'Kea or Kythnos, the near islands.', body: 'Morning sail east. Drop anchor in a bay the ferries do not reach. Swim stops, lunch on board, an afternoon on deck, dinner in a tiny port.' },
    { day: '03', title: 'Serifos and the old mines.', body: 'A morning sail to Serifos. The Chora, high above the sea, a slow walk before it wakes. Back on board for lunch. Evening in a quiet cove.' },
    { day: '04', title: 'The Small Cyclades, barely inhabited.', body: 'Eastward to Iraklia or Schinoussa, islands of a few hundred people. A long swim, a lunch in a taverna that opens for us, an afternoon on a beach nobody else found.' },
    { day: '05', title: 'Amorgos, the cliff island.', body: 'The most dramatic of the Cyclades. The Chora on top, the monastery of Hozoviotissa built into the cliff. A sunset on deck, a night at anchor.' },
    { day: '06', title: 'The open water.', body: 'A long sail, nothing scheduled. The sea, the wind, the deck. Lunch underway. A bay chosen that morning, by conditions, by mood.' },
    { day: '07', title: 'Koufonisia and the last swim.', body: 'The smallest inhabited Cyclade. A morning on the most perfect beach in Greece, according to the people who know. A slow lunch. The last sunset.' },
    { day: '08', title: 'Back to the mainland.', body: 'A sail back to Athens or a nearby port. Disembarkation mid-morning. Private transfer to the airport, assisted transit.' },
  ];

  const includedItems = [
    'Seven nights on board a private yacht',
    'Professional crew (captain, mate, hostess, chef)',
    'Chef on board, all meals',
    'Daily concierge by the eb. studio',
    'Route designed around you, weather allowing',
  ];

  const onRequestItems = ['Tenders, paddleboards, snorkel gear', 'Signature shoreside experiences', 'Wellness on board'];
    ...
    <main className="flex flex-col min-h-screen bg-white">
      ...
      <div className="absolute top-[88px] md:top-[96px] left-6 md:left-12 z-40">
        <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-[10px] md:text-[11px] tracking-[0.25em] uppercase text-white/75 font-light">
          ...
          <span className="opacity-70">A Week in the Cyclades, by Sea</span>
        </nav>
      </div>

      <section data-nav-dark className="relative w-full h-[75vh] md:h-screen md:min-h-[600px] overflow-hidden eb-image-vignette">
        <Image src="/yatch_page%20collection.jpg" alt="On board a private yacht in the Cyclades" fill priority sizes="100vw" className="object-cover object-center md:object-[center_60%] reveal" />
        ...
        <div className="absolute bottom-14 md:bottom-20 left-6 md:left-12 right-6 md:right-12 text-white">
          ...
          <p className="text-[14px] md:text-[16px] opacity-90 max-w-[480px] leading-[1.55] font-light reveal" data-delay="200">One yacht. Seven mornings. The Aegean to yourselves.</p>
        </div>
        <button type="button" onClick={(e) => (e.currentTarget.closest("section")?.nextElementSibling as HTMLElement)?.scrollIntoView({ behavior: "smooth", block: "start" })} aria-label="Scroll down" className="absolute bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 z-30  cursor-pointer p-2 min-w-[44px] min-h-[44px] flex items-center justify-center"><svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="text-white/60 eb-scroll-chevron"><path d="M5 8L10 13L15 8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </button>
      </section>
        ...
        <div className="max-w-[1200px] mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-10">
            {[
              ...
              { label: 'Destination', value: 'Cyclades, by yacht' },
        ...
        <div className="max-w-[1000px] mx-auto">
          ...
          <div className="flex flex-col items-center">
            <div className="flex flex-col items-center text-center">
              ...
              <div className="text-[17px] md:text-[22px] tracking-[0.25em] uppercase text-[#2e5a88] font-light mb-2">Cyclades, by yacht</div>
              <div className="text-[12px] text-[#1a1a1a]/50 font-light">7 nights &middot; Kea, Serifos, Amorgos, Koufonisia, and what the wind decides</div>
              <div className="text-[10px] tracking-[0.15em] uppercase text-[#1a1a1a]/35 font-light mt-2 italic">From Athens, back to Athens</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-14 md:py-20">
        <div className="max-w-[1400px] mx-auto px-6 md:px-10">
          ...
          <div className="relative h-[60vh] min-h-[400px] md:h-auto md:min-h-0 md:aspect-[21/9] overflow-hidden group reveal eb-image-vignette md:max-w-[1200px] -mx-6 md:mx-auto">
            <Image src="/reveil%20yatch.jpg" alt="Waking up on a private yacht, Aegean morning" fill sizes="100vw" className="object-cover object-center reveal transition-transform duration-700 group-hover:scale-105" />
            ...
            <div className="absolute bottom-16 left-6 right-6 md:bottom-10 md:left-10 md:right-10 text-white z-[3] max-w-[600px]">
              <h3 className="font-anton font-normal uppercase text-[36px] md:text-[40px] leading-[1] mb-4">Waking up somewhere new, every morning.</h3>
              <p className="text-[15px] md:text-[15px] opacity-90 leading-[1.55] font-light">One yacht. A chef on board. A route drawn by wind and mood.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section data-nav-dark className="relative w-full h-[55vh] min-h-[400px] md:h-[80vh] md:min-h-[500px] overflow-hidden eb-image-vignette">
        <Image src="/yatch%20on%20board.jpg" alt="On board the private yacht" fill sizes="100vw" className="object-cover object-center reveal" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/55 via-black/15 to-transparent" />
        <div className="absolute bottom-12 md:bottom-20 left-6 md:left-16 right-6 md:right-16 max-w-[620px] text-white reveal z-[3]">
          ...
          <h2 className="font-anton font-normal uppercase text-[32px] md:text-[56px] leading-[1] md:leading-[0.98] tracking-[0.02em] mb-6">On board.</h2>
          <p className="text-[14px] md:text-[17px] leading-[1.7] md:leading-[1.75] opacity-90 font-light">A hand-selected yacht, crewed, with a chef on board. The right vessel matched to your party, confirmed once we know who is sailing.</p>
        </div>
      </section>

      <section className="bg-white py-10 md:py-14 reveal">
        <div className="max-w-[1280px] mx-auto px-6 md:px-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-3">
            ...
            <div className="relative aspect-[3/4] overflow-hidden eb-image-vignette group"><Image src="/amorgos%202.jpg" alt="Amorgos cliff and sea" fill className="object-cover reveal" /></div>
          ...
          <div className="max-w-[1100px] mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr] gap-10 md:gap-20">
              ...
              <div>
                ...
                <p className="mt-8 text-[12px] md:text-[13px] text-[#1a1a1a]/45 italic font-light leading-[1.6]">Flights, harbor fees, and travel insurance arranged separately.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section data-nav-dark className="relative w-full h-[70vh] min-h-[480px] overflow-hidden eb-image-vignette">
        <Image src="/yatch%20page.jpg" alt="Ready when you are" fill className="object-cover reveal" />
        ...
        <div className="relative z-[3] h-full flex items-center justify-center px-6 md:px-10">
          <div className="text-center max-w-[640px] reveal">
            ...
            <div className="flex justify-center"><Link href="/contact?type=journey&journey=a-week-in-the-cyclades-by-sea" className="eb-cta-link font-body text-[12px] md:text-[13px] tracking-[0.2em] uppercase text-white min-h-[44px] py-3">Start the conversation <span className="eb-cta-arrow">&rarr;</span></Link></div>
          </div>
        </div>
      </section>
      {/* ============ BACK TO JOURNEYS ============ */}
